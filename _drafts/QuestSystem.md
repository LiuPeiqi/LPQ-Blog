...
在上图中有五个关键状态，分别是未接取、进行中、已达成、已结束、已失败。一个隐藏状态已放弃，定义为隐藏状态的原因是已放弃状态可以等同于未接取。然后在这五个状态之间定义了七种状态转移的条件，其中“回退达成条件”情况特殊，既可以设计成“达成条件”的逆条件，也可以设计成新定义的条件类型。状态的切换定义出触发机制：“xx时机”。
...
树结构虽然能提供灵活强大的组织方式，但需要完善的配表工具才能发挥效果。
