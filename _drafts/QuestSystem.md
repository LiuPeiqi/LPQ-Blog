...
当前游戏的任务设计中有多个状态下的进度追踪与导航，比如在任务进行状态下的各个条件的进度与导航，在任务完成状态下的进度追踪（提交地点的提示）与导航。解耦任务与追踪的原因除了上面列举的三条以外，还有一个原因是简化追踪与导航的逻辑：通过任务树（任务的父子关系）来包装一个表现上的任务，那么进度和导航就不必需要多个条件与状态了。比如当前配置：
...
![任务树的组织](../images/quest_system/QuestDisplay.png)
